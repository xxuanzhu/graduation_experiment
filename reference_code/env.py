...
class RoadNet:
    ...
    def generate_node_dict(self):
        '''
        key: node id
        value:dict of input nodes and output nodes
        :return:
        '''

        for node_dict in self.roadnet_dict['intersection']:
            node_id = node_dict['id']  # 道路id
            road_links = node_dict['roads']  # intersection连接的道路
            input_nodes = []  # 进入该node的其他node
            output_nodes = []  # 从该node出去能到的node
            input_edges = []  # 进入该node的边
            output_edges = {}  # 从该node出去的边

            for road_link_id in road_links:
                road_link_dict = self._get_road_dict(road_link_id)
                if road_link_dict['startIntersection'] == node_id:  # 如果这条路连接的起点是node id
            ...
            net_node = {
                ...
                # should be a dict, with key as an input edge, value as output edges
            }
            if node_id not in self.net_node_dict.keys():
                self.net_node_dict[node_id] = net_node

    # 根据road id 在file中 找到road相关信息
    ...
    def generate_edge_dict(self):
        '''
        key: edge id
        value: dict of input edges and output edges
        :return:
        '''
        for edge_dict in self.roadnet_dict['roads']:
            ...
            net_edge = {
                ...
                'input_edges': self.net_node_dict[input_node]['input_edges'],  # 从哪些edges可以进入该edge
                'output_edges': self.net_node_dict[output_node]['output_edges'][edge_id],  # 从该edge可以出去到哪些edges
...
class Env:
    ...
    def reset(self):
        ...
        # cityflow的配置
        cityflow_config = {
            "interval": self.dic_traffic_env_conf["INTERVAL"],  # interval长度
            "seed": 0,
            "laneChange": False,  # 是否变道
            "dir": self.path_to_work_directory + "/",  # 路径
            "roadnetFile": self.dic_traffic_env_conf["ROADNET_FILE"],  # roadnet文件名
            "flowFile": self.dic_traffic_env_conf["TRAFFIC_FILE"],  # flow文件名
            "rlTrafficLight": self.dic_traffic_env_conf["RLTRAFFICLIGHT"],  # 是否通过python API进行红绿灯控制
            "saveReplay": self.dic_traffic_env_conf["SAVEREPLAY"],  # 保存模拟结果去replay
            "roadnetLogFile": "frontend/web/roadnetLogFile.json",  # log文件
            "replayLogFile": "frontend/web/replayLogFile.txt"  # log文件
        }
        print("=========================")
        print(cityflow_config)

        # 写入cityflow.config，相当于cityflow原始的config文件
        with open(os.path.join(self.path_to_work_directory, "cityflow.config"), "w") as json_file:
            json.dump(cityflow_config, json_file)  # python对象转化为json对象
        # cityflow引擎
    ...
    # 日志记录，当前时间，在action之前的feature，以及执行的action
    ...
    def _adjacency_extraction_lane(self):
        ...
        with open('{0}'.format(file)) as json_data:
            ...
            top_k = self.dic_traffic_env_conf["TOP_K_ADJACENCY"]  #top k的数量
            top_k_lane = self.dic_traffic_env_conf["TOP_K_ADJACENCY_LANE"] # top k车道数量
            total_inter_num = len(traffic_light_node_dict.keys())  # 所有intersection的数量
